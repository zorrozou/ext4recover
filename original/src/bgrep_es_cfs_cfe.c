...
int main(int argc, char **argv) {
    ...
    while (nread = read(fd, buf, BLOCK_SIZE)) {
        ...
        if(!started){
            // Check for CFS format
            if ((p = memmem(buf, nread, match_cfs_start, match_cfs_len)) == buf) {
                sprintf(wfilename, "%s/%lu.cfs", RECOVER_DIR, blk_number);
            }
            // Check for CFE format
            else if ((p = memmem(buf, nread, match_cfe_start, match_cfe_len)) == buf) {
                sprintf(wfilename, "%s/%lu.cfe", RECOVER_DIR, blk_number);
            }
            // Check for other formats like fdt, fdx, fnm, si, etc. (you can uncomment and modify this part)
            /*
            else if (!started && ((p = memmem(buf, nread, match_fdt1_start, match_fdt1_len)) == buf)) {
                sprintf(wfilename, "%s/%lu.fdt", RECOVER_DIR, blk_number);
            }
            else if (!started && ((p = memmem(buf, nread, match_fdx_start, match_fdx_len)) == buf)) {
