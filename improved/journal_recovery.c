                                       ...
                                       struct ext2_inode_large *jinode)
{
    ...
    if (depth != 0) {
            ...
            if (ceh->eh_depth == 0) {
                ...
                for (int j = 0; j < leaf_entries; j++, cee++) {
                    ...
                    if (recover_block_to_file(ctx->device_fd, fd_ml,
                                            blk_num, blk_len, phys, ctx->blocksize)) {
        ...
        if (recovered_blocks_ml > 0) {
            ...
            /* Only replace if this recovery produced more data */
            struct stat exist_st;
            int do_replace = 1;
            if (stat(filename_ml, &exist_st) == 0 && exist_st.st_size > 0) {
                struct stat tmp_st;
                if (stat(tmpname_ml, &tmp_st) == 0 && tmp_st.st_size <= exist_st.st_size) {
                    do_replace = 0;
                }
            }
            if (do_replace) {
    ...
    if (recovered_blocks > 0) {
        ...
        /* Only replace if this is larger */
        struct stat exist_st;
        int do_replace = 1;
        if (stat(filename, &exist_st) == 0 && exist_st.st_size > 0) {
            struct stat tmp_st;
            if (stat(tmpname, &tmp_st) == 0 && tmp_st.st_size <= exist_st.st_size) {
                do_replace = 0;
            }
        }
        if (do_replace) {
                                      ...
                                      char *buf, blk64_t fs_block)
{
    ...
    for (int i = 0; i < inodes_in_block; i++) {
        ...
        /* Skip if already recovered with equal or larger size */
        if (already_recovered_larger(state, ino, file_size))
            continue;
        
        LOG_DEBUG(ctx, "Found deleted inode %u in journal (size=%llu, entries=%u, depth=%u)",
                 ino, (unsigned long long)file_size, 
                 ext2fs_le16_to_cpu(eh->eh_entries),
                 ext2fs_le16_to_cpu(eh->eh_depth));
        
        /* Try to recover */
        if (recover_inode_from_journal(ctx, ino, jinode) == 0) {
            mark_recovered(state, ino, file_size);
            recovered++;
        }
    }
...
int recover_from_journal(struct recover_context *ctx)
{
    ...
    if (!state.recovered_inodes || !state.recovered_sizes) {
        ...
        free(state.recovered_sizes);
        return -1;
    }
    
    retval = ext2fs_get_mem(ctx->blocksize, &desc_buf);
    if (retval) { free(state.recovered_inodes); free(state.recovered_sizes); return -1; }
    
    retval = ext2fs_get_mem(ctx->blocksize, &data_buf);
    if (retval) { ext2fs_free_mem(&desc_buf); free(state.recovered_inodes); free(state.recovered_sizes); return -1; }
    ...
    for (blk = ctx->journal_start; blk < ctx->journal_len; blk++) {
        ...
        while (remaining >= (int)sizeof(journal_block_tag_t)) {
            ...
            if (is_inode_table_block(&state, fs_block)) {
                process_inode_table_block(&state, data_buf, fs_block);
